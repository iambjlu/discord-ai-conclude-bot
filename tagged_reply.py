...
以下是近期對話歷史（僅供背景參考）：
{context_str}""",
        "MODEL_PRIORITY_LIST": ["gemini-3.1-flash-lite-preview","gemma-4-31b-it"],
...
class TaggedResponseBot(discord.Client):
    def __init__(self, settings, secrets, *args, **kwargs):
        ...
        self.model_priority_list = self.settings.get("MODEL_PRIORITY_LIST", ["gemini-3.1-flash-lite-preview","gemma-4-31b-it"])
                ...
                async with message.channel.typing():
                    try:
                        ...
                        if is_smarter_mode:
                            ...
                            model_name = self.settings.get("MODEL_PRIORITY_LIST", ["gemini-3.1-flash-lite-preview","gemma-4-31b-it"])[0]
