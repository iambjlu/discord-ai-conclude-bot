...
class MyClient(discord.Client):
    async def on_ready(self):
        ...
        ai_summary = True # AI總結: True=開啟(每次都執行), False=關閉(永遠不執行)
        link_screenshot = True # 連結截圖: True=開啟(每次都執行), False=關閉(永遠不執行)
...
"""
        ...
        # --- 新增功能：AI重點摘要模組 ---
        if not ai_summary:
            print(f"AI 總結功能已關閉 (ai_summary=False)，跳過。")
        else:
        ...
        # --- 新增功能：連結截圖模組 ---
        if not link_screenshot:
             print(f"連結截圖功能已關閉 (link_screenshot=False)，跳過。")
        else:
